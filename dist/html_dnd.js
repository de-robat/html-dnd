...
(function (dnd) {
    "use strict";
    function simulate(draggable, droppable, config) {
        if (config === void 0) { config = { onlyHover: false, hoverTime: 0 }; }
        if (!config.onlyHover) {
    ...
    dnd.simulateDragAndDrop = simulateDragAndDrop;
    function simulateDragOver(draggable, droppable, hoverTime) {
        if (hoverTime === void 0) { hoverTime = 0; }
        var store = new DragDataStore();
        // For the dragstart event. New data can be added to the drag data store.
        store.mode = "readwrite";
        var dataTransfer = new DataTransfer(store);
    ...
    function drop(droppable, dataTransfer) {
        var dropEvent = createEventWithDataTransfer("drop", dataTransfer);
        droppable.dispatchEvent(dropEvent);
    }
    function dragLeave(droppable, dataTransfer) {
        var dragExitEvent = createEventWithDataTransfer("dragexit", dataTransfer);
        droppable.dispatchEvent(dragExitEvent);
        var dragLeaveEvent = createEventWithDataTransfer("dragleave", dataTransfer);
        droppable.dispatchEvent(dragLeaveEvent);
    }
    function endDrag(store, draggable, dataTransfer) {
